...
pnpm 9+ 默认禁用 install scripts。必须在根 `package.json` 加：
```json
...
| **better-sqlite3 / native modules in pnpm 9+** | （非版本变化，是 pnpm 行为变化） | pnpm 9+ 默认禁所有 install scripts | 根 `package.json` 必须有 `pnpm.onlyBuiltDependencies: ["better-sqlite3"]` |
...
| pnpm 9+ 对 `onlyBuiltDependencies` 的支持 | ✓ 自 pnpm 9.4 起 GA | 模板锁 `packageManager: pnpm@9.12.0` 或更新 |
